...
function ForceGraph({ nodes, edges, focusNode, onNodeClick }: { nodes: GNode[]; edges: GEdge[]; focusNode: string | null; onNodeClick?: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    ng.append('circle')
      .attr('r', (d: any) => NODE_R[d.type] || 4)
      .attr('fill', (d: any) => d.id === focusNode ? '#ef4444' : (NODE_COLOR[d.type] || '#999'))
      .attr('stroke', '#fff').attr('stroke-width', 1.5)
    if (focusNode) {
      ng.selectAll('circle').filter((d: any) => d.id === focusNode).attr('r', 10)
    }
    ng.append('text').text((d: any) => (d.label || '').slice(0, 12)).attr('dx', 9).attr('dy', 4).style('font-size', '8px').style('fill', '#555')
