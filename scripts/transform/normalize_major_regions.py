...
from scripts.transform.major_normalization import normalize_major_region_record
...
def run_normalize_major_regions(
    input_path: str | Path,
    ...
    for record in records:
        normalized_record = normalize_major_region_record(_unwrap_payload(record))
...
def main() -> None:
    parser = build_common_parser("Normalize major region records.")
    args = parser.parse_args()
    report = run_normalize_major_regions(
        input_path=args.input,
        output_path=args.output,
        config_path=args.config,
        run_id=args.run_id,
    )
    print(f"normalize_major_regions done: {report['output_records']}")
